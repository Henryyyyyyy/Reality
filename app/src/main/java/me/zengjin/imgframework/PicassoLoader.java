...
import me.RealityApp;
import me.zengjin.zj_basemodule.image.loader.BitmapCallBack;
import me.zengjin.zj_basemodule.image.loader.ILoaderStrategy;
import me.zengjin.zj_basemodule.image.loader.LoaderOptions;
...
public class PicassoLoader implements ILoaderStrategy {
	...
	private static LruCache sLruCache = new LruCache(RealityApp.gApp);

	private static Picasso getPicasso() {
		if (sPicassoSingleton == null) {
			synchronized (PicassoLoader.class) {
				if (sPicassoSingleton == null) {
					sPicassoSingleton = new Picasso.Builder(RealityApp.gApp).memoryCache(sLruCache).build();
				}
			}
		}
	...
	public void clearDiskCache() {
		File diskFile = new File(RealityApp.gApp.getCacheDir(), PICASSO_CACHE);
		if (diskFile.exists()) {
			//这边自行写删除代码
